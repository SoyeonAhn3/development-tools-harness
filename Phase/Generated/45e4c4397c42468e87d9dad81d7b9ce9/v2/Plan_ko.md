...
Phase 3의 실제 순차 흐름, 근거 기록, 수정·재개, 최소 결과 안내와 고정 실행본 A를 위한 외부 예제 통과 조건을 상세화한다. 이번 계획 기능 실행에서 기본 검사 156개가 통과하고 실제 Planner 호출 1회로 v1을 생성했다. v2는 기록된 수동 검토 보완을 포함한다. Phase 2 사용자 인수는 대기 중이다. 실제 작업 프로세스 연결 전에 권한 수단을 선정·시험하며, 제어 코드 개발과 임시 대상을 사용한 경계 실험으로 그 근거를 준비할 수 있다. 이 계획은 Phase 3 구현이나 인수 완료를 뜻하지 않는다.
...
### Phase4 — Phase 4 — 자체 개발과 MVP 인수

검증된 A로 남은 작은 P0 보고서 기능 하나를 개발하고 보고서 집계를 완성하며 준비된 신규·기존 외부 프로젝트를 검증한다.

완료 기준: 자체 개발 인수, 신규·기존 외부 프로젝트 각각의 Phase 인수, 핵심 회귀 검사와 축소 MVP 11장 전체 기준의 근거를 확보한다. Phase 3 예제는 근거가 일치할 때 해당 유형으로 인정할 수 있다. 상세 Task는 Phase 3 결과로 작성한다.

## 현재 Phase Task: Phase3

### P3-T1 — 선행 조건 확인·작업 권한 분리 검토·실행 계약과 예제 정의
...
완료 기준: Phase 3 착수 전에 Phase 2 사용자 인수를 기록한다. 임시 대상을 사용해 Windows 권한 수단을 조사·선정하고 한계를 기록하며 실제 시험 전에는 분리를 검증했다고 하지 않는다. 현재 Phase Task·검사·설정 가능한 수정 한도(제안 기본값 2회)를 정의한다. Q2는 실제 작업 프로세스 연결의 조건이며 제어 코드와 임시 대상 시험 개발로 필요한 근거를 확보할 수 있다. 제안 workflow_project 파일은 정수를 두 배로 출력하는 CLI와 양수·0 기본 검사다. 한정된 변경은 정상 출력을 유지하면서 음수에 명확한 오류와 0이 아닌 종료 코드를 반환한다. 실제 예제 전에 명령과 기대 출력을 기록한다.

검증 방법: Phase 2 사용자 인수를 확인하고 제안 권한 수단과 임시 대상 시험 결과 또는 미해결 한계를 기록한다. 임시 예제 복사본에서 python -m pytest -q로 기본 검사를 수행하고 실행 계약을 Phase 3와 대조한다. P3-T2 권한 근거가 통과하기 전에는 실제 대상을 수정하는 작업 프로세스를 실행하지 않는다.
...
완료 기준: Q2에서 확정한 경계로 선정된 Adapter의 실제 Developer와 별도로 생성한 Reviewer 세션을 지원한다. 작업 권한은 승인된 대상 내용 수정으로 제한하고 제어 실행본의 승인·정책 저장소 접근을 차단하며 테스트 자손 프로세스에도 경계를 적용한다. 기존 텍스트 전용 Planner 경계를 유지한다. 통합 테스트 경로는 신규 제안이다. 활성 실행 기록을 직접 편집하지 않는다. Reviewer는 읽기 전용으로 둔다. Developer 쓰기는 승인된 대상 경로에 한정한다. 모든 역할과 검사 자손 프로세스의 A·활성 정책 및 승인 기록·무관한 파일·Git 메타데이터 쓰기, 설치와 승인되지 않은 외부 전송을 차단한다. 역할에 필요한 선정 AI 서비스 통신만 허용한다. 기존 축소 MVP의 이 제약을 실행 계약에 반영한다.

검증 방법: 실제 작업·자손 프로세스와 폐기 가능한 보호 대상을 사용해 허용된 대상 변경과 보호 영역 쓰기의 사전 차단을 입증한다. 프롬프트나 가짜 응답만이 아니라 선정된 실제 CLI의 역할 경계를 검증한다. Planner 기능·호환성 회귀 검사를 수행하며 경계가 없으면 실제 흐름을 차단한다. 임시 예제에서 Reviewer 쓰기·범위 밖 쓰기·Git 메타데이터 쓰기·설치 시도·승인되지 않은 외부 연결을 시험하고 영향이 없으며 제약이 상속되는지 확인한다. 집행할 수 없는 동작은 실행을 막는다.
...
완료 기준: 검토한 계획과 시작 내용 식별자를 참조하는 제어 실행본 소유의 실행 진입을 추가하고 계획 전용 승인과 명시적 실행 승인을 구분한다. 고유 역할 시도·실행 여부·세션 ID·확정 및 미확정 시간·승인 대기·지적 ID를 원자적으로 저장한다. 근거를 계획·코드 버전에 연결하며 사용량은 관측값만 기록한다. 단위 테스트 경로는 신규 제안이다. 지적 기록에는 중요도·내용·미해결/해결/오탐/보류 처리 상태와 근거를 포함하고 재검토에서도 식별자를 유지한다.
...
검증 방법: 순서가 있는 Task 2개·반복 지적·거짓 수정 보고·필수 지적 보류·한도 0회와 2회·한도 소진을 검사한다. 실제 pytest로 실패·생략 및 혼합 테스트·테스트 0개·시간 초과·도구 누락을 확인한다. 검사 미충족 시 진행하지 않고 최종 근거를 최종 내용과 대조한다. 중요도·처리 이력을 확인하고 오탐·해결 분류에 근거를 요구하며 보류된 필수 지적도 완료를 막는지 검증한다.
...
- `src/development_harness/validation.py` (explicit): 검증은 프로젝트 명령 실행 전에 프로세스 식별자를 저장한다. 기존 결과 분류는 실제 흐름 연결에서 재검증할 기반이다.
...
- Q1 (blocking): Phase 2 사용자 인수는 대기 중이다. 이번 실행으로 Task 2.6 계획 기능 근거(기본 검사 156개, 실제 Planner 호출 1회, 검토한 v2)를 확보했지만 기술적 결과가 사용자 인수를 대신하지 않는다. Phase 3 착수 전에 사용자 판단을 기록한다.

- Q2 (blocking): Developer·테스트가 A와 활성 실행·승인 저장소를 변경하지 못하게 할 Windows 수단은 아직 선정·검증되지 않았다. P3-T1에서 검토하고 P3-T2에서 허용·차단의 실제 시험 근거를 확보한 후 실제 작업 프로세스를 연결한다. 현재 사용자에게 권한 수단 선정을 요청하는 것이 아니라 개발에서 해결할 기술 검토 사항이다.
