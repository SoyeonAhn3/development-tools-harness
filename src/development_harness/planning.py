...
from .plan_schema import SCHEMA, render, validate_plan
...
class Planning:
    ...
    def plan(self, *, baseline_only=False, adapter_factory=CodexPlanner):
        with ProjectLock(self.store):
            run = self.get()
            ...
            try:
                ...
                adapter = adapter_factory(run["config"]["model"], call_directory)
                ...
                response = adapter.generate(prompt, SCHEMA, attempt, lambda a: self._launched(run, a),
    ...
    def _publish(self, run, plan, *, source, attempt_id=None):
        ...
        artifacts = {directory + "/plan.json": json.dumps(plan, ensure_ascii=False, indent=2) + "\n",
                     directory + "/Plan.md": render(plan, "en"), directory + "/Plan_ko.md": render(plan, "ko")}
    ...
    def revise(self, path):
        with ProjectLock(self.store):
            ...
            files = self._check(run)
            self._require_evidence(run)
            plan = validate_plan(read_json(path), files)
